...
- **Gestión de usuarios (crear/editar/listar en `/admin/usuarios`):** `perfiles` no tiene policy RLS de escritura para admin, así que crear y editar usuarios pasan por el cliente `supabaseAdmin` (`service_role_key`) y el listado por la RPC `get_all_perfiles` — detalle completo, incluida la nota de riesgo por exponer la `service_role_key` en el bundle del navegador, en `06_estructura_de_datos.md` (sección RLS).
